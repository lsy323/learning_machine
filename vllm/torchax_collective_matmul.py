...
class FeedForwardLinear(torch.nn.Module):
    ...
    def shard_weight(self, row_sharding, col_sharding):
        self.w1.weight.data = self.w1.weight.data.apply_jax(jax.device_put, row_sharding)
        self.w2.weight.data = self.w2.weight.data.apply_jax(jax.device_put, col_sharding)
        self.w3.weight.data = self.w3.weight.data.apply_jax(jax.device_put, row_sharding)
...
def apply_sharding(tensor, mesh, sharding_spec):
    """Apply sharding to a tensor based on the given mesh and sharding spec."""
    return tensor.apply_jax(jax.lax.with_sharding_constraint, NamedSharding(mesh, sharding_spec))

class FeedForwardEinsum(torch.nn.Module):
    """FeedForward using torch.einsum for matrix multiplications."""

    def __init__(self, input_dim, hidden_dim, mesh=None):
    ...
    def shard_weight(self, row_sharding, col_sharding):
        self.w1.data = self.w1.data.apply_jax(jax.device_put, col_sharding)
        self.w2.data = self.w2.data.apply_jax(jax.device_put, row_sharding)
        self.w3.data = self.w3.data.apply_jax(jax.device_put, col_sharding)

    def forward(self, x):
        x = apply_sharding(x, self.mesh, P('x', None))
        ...
        # w1_proj = apply_sharding(w1_proj, self.mesh, P(None, 'x')) if self.mesh else w1_proj
        # w3_proj = apply_sharding(w3_proj, self.mesh, P(None, 'x')) if self.mesh else w3_proj
        act = F.silu(w1_proj * w3_proj)
        # act = F.silu(w1_proj)
        res = torch.einsum('...h,hi->...i', act, self.w2)    # [..., hidden_dim] @ [hidden_dim, input_dim] -> [..., input_dim]
        res = apply_sharding(res, self.mesh, P('x', None))
        return res
...
with torch.no_grad():
    ffn = FeedForward(model_dim, hidden_dim, mesh=mesh)
...
row_sharding = NamedSharding(mesh, P(None, 'x'))
col_sharding = NamedSharding(mesh, P('x', None))

# Place x on the device mesh with sharding
x = x.apply_jax(jax.device_put, NamedSharding(mesh, P('x', None)))
...
# ffn.w1.data = ffn.w1.data.apply_jax(jax.device_put, replication_sharding)
# ffn.w3.data = ffn.w3.data.apply_jax(jax.device_put, replication_sharding)
# ffn.w2.data = ffn.w2.data.apply_jax(jax.device_put, replication_sharding)
# ffn.shard_weight(row_sharding, col_sharding)
print("FFN weights sharded:")
print(f"w1: {ffn.w1.data.jax().sharding}")
print(f"w2: {ffn.w2.data.jax().sharding}")
print(f"w3: {ffn.w3.data.jax().sharding}")
...
end_time = time.time()
print(f"Average time per iteration: {(end_time - start_time) / num_iterations * 1000:.4f} ms")
