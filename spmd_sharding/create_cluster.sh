...
CLUSTER_NAME=hanq-pytorch-${TPU_TYPE}-$ZONE
...
CLUSTER_ARGUMENTS="--network=$NETWORK_NAME --subnetwork=$SUBNET_NAME"

xpk cluster create --cluster $CLUSTER_NAME --num-slices=$NUM_SLICES --tpu-type=$TPU_TYPE --zone=$ZONE --project=$PROJECT --custom-cluster-arguments="${CLUSTER_ARGUMENTS}" --reservation=$reservation
